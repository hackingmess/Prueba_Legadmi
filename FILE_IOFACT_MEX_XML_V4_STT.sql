CREATE OR REPLACE PACKAGE FILE_IOFACT_MEX_XML_V4_STT AS
--SONOCO
...
PROCEDURE GENERAR_IOFACT_MEX_XML (PEMPRESA               IN   NUMBER,
                                  ...
                                  PREGIMEN_FISCAL_RECEPTOR IN VARCHAR2,
                                  PNO_ERROR                OUT NUMBER,
                                  PMG_ERROR                OUT VARCHAR2,
                                  PFECHA_INI_TIMB          IN   DATE,
                                  PFECHA_FIN_TIMB          IN   DATE,
                                  PSALARIO_BASE_COT        IN   VARCHAR2);                                        
END FILE_IOFACT_MEX_XML_V4_STT;
/

CREATE OR REPLACE PACKAGE BODY FILE_IOFACT_MEX_XML_V4_STT IS
--
FUNCTION COD_CONCEPTO_CATALOGO_IOPAC (PCOD_EMPRESA               IN   NUMBER,
                                      PCOD_TRABAJADOR            IN   NUMBER,
                                      PFECHA_D                   IN   DATE, 
...
END;
--
--Se agregó una función para que tome el concepto dependiendo de lo que este parametrizado en la columna COD_REGIMEN de la tabla CODIFICACION_CONC_GOSOCKET
--
FUNCTION MONTO_POR_GRUPO_REGIMEN      (PCOD_EMPRESA               IN   NUMBER,
                                       PCOD_TRABAJADOR            IN   NUMBER,
                                       PFECHA_D                   IN   DATE, 
...
--
--Se agregó un nuevo parámetro de entrada: PREGIMEN
--
...
BEGIN
   ...
   MLINEA := MLINEA||'<DTE version="4.0">'||CHR(13);
   UTL_FILE.PUT_LINE(MFILE, MLINEA);                        
   /*REGISTROS1*/ 
   --
   --El select anterior se agregó como una tabla llamada QUERY1 y se agregó la tabla CODIFICACION_CONC_GOSOCKET
   --Se llama la nueva función en algunos de los cmpos para que calcule dependiendo del Cod_regimen la nueva funcion se llama "MONTO_POR_GRUPO_REGIMEN"
   --
   FOR CLIE IN (SELECT   QUERY1.VERSION_CFDI
                        ...
                        ,MONTO_POR_GRUPO_REGIMEN(QUERY1.cod_empresa,QUERY1.cod_trabajador,PFECHA_D,PFECHA_H,PPROCESO,PCONCEPTO_VALOR_UNITARIO,CG.COD_REGIMEN,QUERY1.COD_CONTRATO,'P','O') CONCEPTO_VALOR_UNITARIO                        -- Campo 28
                        ,TRIM(TO_CHAR(MONTO_POR_GRUPO_REGIMEN(QUERY1.cod_empresa,QUERY1.cod_trabajador,PFECHA_D,PFECHA_H,PPROCESO,PCONCEPTO_IMPORTE,CG.COD_REGIMEN,QUERY1.COD_CONTRATO,'P','D','O'),'999999999999999999.99')) CONCEPTO_IMPORTE  -- Campo 29
                        ...
                        ,MONTO_POR_GRUPO_REGIMEN(QUERY1.cod_empresa,QUERY1.cod_trabajador,PFECHA_D,PFECHA_H,PPROCESO,PSI_INGRESO_ACUM,CG.COD_REGIMEN,QUERY1.COD_CONTRATO,'P') SI_INGRESO_ACUM                                        -- Campo 193
                        ...
                        ,MONTO_CONCEPTO_OTROS_PAGOS_REGIMEN (QUERY1.cod_empresa,QUERY1.cod_trabajador,PFECHA_D,PFECHA_H,PPROCESO,1,QUERY1.COD_CONTRATO,'N','O','S',PREGIMEN) OP_IMPORTE_1
                        ,QUERY1.FECHA_ANTI_FINAL
                        ,QUERY1.FECHA_ANTI_INICIAL
                        ,QUERY1.CODIGO_POSTAL
                        ,QUERY1.NOMBRE_SUCURSAL 
                FROM CODIFICACION_CONC_GOSOCKET CG,
                     --Select anterior
                    (SELECT '4.0' VERSION_CFDI,                                                                                                                          -- Campo 1
                            PSERIE SERIE,                                                                                                                                       -- Campo 2
                            PFORMA_PAGO FORMA_PAGO,                                                                                                                             -- Campo 4            -- Campo 8                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         
...
--                            TRIM(TO_CHAR(MONTO_POR_GRUPO(t.cod_empresa,t.cod_trabajador,PFECHA_D,PFECHA_H,PPROCESO,PCONCEPTO_DESCUENTO),'999999999999999999.99')) CONCEPTO_DESCUENTO, -- Campo 30
                            ...
                            DM.TIPO_REGIMEN_SAT R_TIPO_REGIMEN,                                                                                                                 -- Campo 71
                            ...
                            SUEL.SUELDO,                                                                                                      -- Campo 79
--                            MONTO_POR_GRUPO(t.cod_empresa,t.cod_trabajador,PFECHA_D,PFECHA_H,PPROCESO,PSALARIO_DIARIO_INTEGRADO) R_SALARIO_DIARIO_INTEGRADO,                                        -- Campo 80
    ...
    --
     IF PSI_INGRESO_ACUM IS NULL AND PSI_INGRESO_NO_ACUM  IS NULL 
        ...
        /*ANTIG_TOTAL(CLIE.COD_TRABAJADOR,
                    ...
                    VMES,
                    VDIA);*/
        FILE_IOFACT_MEX_XML_V4_STT.ANTIGUEDAD_TOTAL ( CLIE.FECHA_ANTI_FINAL,
                                                      CLIE.FECHA_ANTI_INICIAL,
                                                      VANO,
                                                      VMES,
                                                      VDIA);            
        ...
        MLINEA := MLINEA||' RegimenContableR="' || PREGIMEN_FISCAL_RECEPTOR || '"';
        ...
        MLINEA := MLINEA||'<DocRecep';   
           ...
           MLINEA := MLINEA||' UsoCFDI="CN01"';
           MLINEA := MLINEA||' DomicilioFiscalReceptor="'||CLIE.CODIGO_POSTAL || '"';
       ...
       ---
        ...
        MLINEA := MLINEA||' TipoRegimen="'||NVL(CLIE.COD_REGIMEN,CLIE.R_TIPO_REGIMEN)||'"';                     --Se cambió la lógica del valor de TipoRegimen
        ...
        MLINEA := MLINEA||' SalarioDiarioIntegrado ="'||TRIM(TO_CHAR(CLIE.R_SALARIO_DIARIO_INTEGRADO,'999999999999999990.00'))||'"';
        MLINEA := MLINEA||' SalarioDiario ="'||TRIM(TO_CHAR(CLIE.R_SALARIO_DIARIO,'999999999999999990.00'))||'"';
       ...
       --
        ...
        MLINEA := MLINEA || '<Planta>' || '"' || NVL(CLIE.NOMBRE_SUCURSAL, '') || '"' || '</Planta>' || CHR(13);
        MLINEA := MLINEA || '</Personalizados>' || CHR(13);
        --*************************************************************************************************
        --******************FINAL TO TODO 
        MLINEA := MLINEA||'</Payroll>'||CHR(13);
